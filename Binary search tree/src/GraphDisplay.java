...
class GraphDisplay extends JComponent
{
	...
	{
		//create a random graph with n=20 and p=0.3
		int n = 20;
		Graph g = new Graph();
		g.randomGraph(n, 0.3);
		...
		for(int i=1; i<=n; i++){
			Point p = g.getPosition(i-1);
			gd.addNode(i,p.x, p.y);
		}
		...
		//ex 8 ex
		gd = new GraphDisplay();
		gd.showInWindow(400,400,"Prim");
		for(int i=1; i<=n; i++){
			Point p = g.getPosition(i-1);
			gd.addNode(i,p.x, p.y);
		}
		for(Graph.Edge e: g.prim()){
			int a = e.n1+1;
			int b = e.n2+1;
			gd.addEdge(a,b,Color.red);
		}
		...
		//ex 9 ex
		gd = new GraphDisplay();
		gd.showInWindow(400,400,"Kruscal");
		for(int i=1; i<=n; i++){
			Point p = g.getPosition(i-1);
			gd.addNode(i,p.x, p.y);
		}
		for(Graph.Edge e: g.kruscal()){
			int a = e.n1+1;
			int b = e.n2+1;
			gd.addEdge(a,b,Color.red);
		}
